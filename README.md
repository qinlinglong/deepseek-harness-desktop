...
![本地模式-即可启动直接打开使用](assets/dsh-pic-本地模式-即可启动直接打开使用.jpg)
...
![局域网服务登录认证界面](assets/dsh-pic-局域网服务登录认证界面.jpg)
...
![悬浮图标和迷你窗口](assets/dsh-pic-悬浮图标和迷你窗口.jpg)
...
1. **密码门禁**：应用在 `0.0.0.0:<局域网端口>` 启动一个反向代理，未携带有效登录 Cookie 的请求一律返回登录页。密码以 `sha256(salt+password)` 形式存储（config.json），登录比较使用恒定时间算法（`crypto.timingSafeEqual`）。
